...
Create faculty:
```
curl -s -X POST http://localhost:3001/api/v1/admin/faculties \
...
List faculties:
```
curl -s "http://localhost:3001/api/v1/admin/faculties?limit=20&offset=0" \
  -H "Authorization: Bearer $TOKEN"
```

Assign faculty to user:
```
curl -s -X PATCH http://localhost:3001/api/v1/admin/users/<USER_ID>/faculty \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"facultyId":1}'
```
...
```
curl -s http://localhost:3001/api/v1/guest/faculties
```

Guest selected by faculty:
```
curl -s http://localhost:3001/api/v1/guest/faculties/1/contributions/selected
```

Coordinator list:
```
curl -s http://localhost:3001/api/v1/coordinator/contributions \
  -H "Authorization: Bearer <COORD_TOKEN>"
```

Reports:
```
curl -s "http://localhost:3001/api/v1/reports/faculty/1/statistics?academicYearId=2024" \
  -H "Authorization: Bearer $TOKEN"
```
...
```
curl -s -X PATCH http://localhost:3001/api/v1/admin/users/<STUDENT_ID>/faculty \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"facultyId":1}'
```
...
4) Update valid:
```
curl -s -X PATCH http://localhost:3001/api/v1/admin/faculties/1 \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
5) Invalid id:
```
curl -i -X PATCH http://localhost:3001/api/v1/admin/faculties/abc \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Invalid"}'
```
...
6) Deactivate:
```
curl -s -X PATCH http://localhost:3001/api/v1/admin/faculties/1/deactivate \
  -H "Authorization: Bearer $TOKEN"
```
Expected: `200`
...
```
curl -i http://localhost:3001/api/v1/admin/faculties \
...
```
curl -s -X PATCH http://localhost:3001/api/v1/admin/users/<COORD_ID>/faculty \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"facultyId":1}'
```
